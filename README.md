...
`tap_animation_name` guarda el nombre exacto de la animación del GLB que se reproduce al tocar una zona libre de la pantalla en Unity. Vacío conserva la selección automática de `Walk`/`Caminar` al tocar el modelo. El panel obtiene los nombres del visor GLB; la API limita la longitud, pero no inspecciona el archivo para verificar que el clip exista. Al retirar o sustituir un GLB sin enviar un nuevo nombre, se borra la selección anterior.
